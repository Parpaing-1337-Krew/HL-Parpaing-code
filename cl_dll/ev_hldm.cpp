...
int g_iParpaingSwing;

//Only predict the miss sounds, hit sounds are still played 
//server side, so players don't get the wrong idea.
void EV_Parpaing( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	//Play Swing sound
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/parpaing_miss.wav", 1, ATTN_NORM, 0, PITCH_NORM); 

	if ( EV_IsLocal( idx ) )
	{
		gEngfuncs.pEventAPI->EV_WeaponAnimation( PARPAING_ATTACK1, 1 );

		switch( (g_iParpaingSwing++) % 3 )
		{
			case 0:
				gEngfuncs.pEventAPI->EV_WeaponAnimation ( PARPAING_ATTACK1, 1 ); break;
			case 1:
				gEngfuncs.pEventAPI->EV_WeaponAnimation ( PARPAING_ATTACK1, 1 ); break;
			case 2:
				gEngfuncs.pEventAPI->EV_WeaponAnimation ( PARPAING_ATTACK1, 1 ); break;
...
void EV_Sifflet2Fire( event_args_t *args )
{
		return;
	/*int idx;
	...
	if ( tr.allsolid == 0 && tr.startsolid == 0 && tr.fraction > 0.25 )
		 gEngfuncs.pEventAPI->EV_WeaponAnimation ( SIFFLET2_ATTACK, 0 );
...
void EV_CarnetFire( event_args_t *args )
{
	return;
	...
	//Find space to drop the thing.
	if ( tr.allsolid == 0 && tr.startsolid == 0 && tr.fraction > 0.25 )
		 gEngfuncs.pEventAPI->EV_WeaponAnimation ( CARNET_ATTACK, 0 );
