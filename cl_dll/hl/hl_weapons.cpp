...
CCrowbar g_Crowbar;
CParpaing g_Parpaing;
CSifflet2 g_Sifflet2;
CCarnet g_Carnet;
...
void HUD_InitClientWeapons( void )
{
	...
	HUD_PrepEntity( &g_Parpaing	, &player );
	HUD_PrepEntity( &g_Sifflet2	, &player );
	HUD_PrepEntity( &g_Carnet	, &player );
}
...
void HUD_WeaponsPostThink( local_state_s *from, local_state_s *to, usercmd_t *cmd, double time, unsigned int random_seed )
{
	...
	switch ( from->client.m_iId )
	{
		...
		case WEAPON_PARPAING:
			pWeapon = &g_Parpaing;
			break;

		case WEAPON_SIFFLET2:
			pWeapon = &g_Sifflet2;
			break;
