...
//
// $Workfile:     $
// $Date: 2003/04/28 20:45:48 $
//
//-----------------------------------------------------------------------------
// $Log: vgui_MOTDWindow.cpp,v $
// Revision 1.1  2003/04/28 20:45:48  madfab
// *** empty log message ***
//
//
// $NoKeywords: $
...
void CMessageWindowPanel::UpdateTime(int m_iMin , int m_iSec)
{
	gEngfuncs.Con_DPrintf( "%i:%i\n",m_iMin,m_iSec  ); // equivalent de ALERT mais coté client
}

//-----------------------------------------------------------------------------
// Purpose: Constructs a message panel
//-----------------------------------------------------------------------------
CMessageWindowPanel::CMessageWindowPanel( const char *szMOTD, const char *szTitle, int iShadeFullscreen, int iRemoveMe, int x, int y, int wide, int tall ) : CMenuPanel(0, iRemoveMe, x, y, wide, tall )
{
	// Get the scheme used for the Titles
	CSchemeManager *pSchemes = gViewPort->GetSchemeManager();

	// schemes
	SchemeHandle_t hTitleScheme = pSchemes->getSchemeHandle( "Info Font" );
...
//	int r, g, b, a;
	...
	m_pBackgroundPanel->setBgColor(255, 255, 255, 255);
...
*/
	...
	/*m_pImage = new CImageLabel ("mac10.tga",0,0,100,100);
	m_pImage->setParent( this );
	m_pImage->setVisible( true );
*/
/*				m_pClassImages = new CImageLabel( "mac10.tga", 0, 0, 100, 100 );
				CImageLabel *pLabel = m_pClassImages;
