...
//				pFlag->SetTouch(pFlag->Touch);
	...
	if (m_iActiveFlags > 1)
		UTIL_MessageEffectAll( UTIL_VarArgs( "Capture %i bouteilles", m_iActiveFlags), 1, -1, -1, 0, Vector(0, 255, 0), 255, Vector(0,0,0),0,1,1,1,0);
	else if (m_iActiveFlags == 1)
		UTIL_MessageEffectAll( UTIL_VarArgs( "Capture %i bouteille", m_iActiveFlags), 1, -1, -1, 0, Vector(0, 255, 0), 255, Vector(0,0,0),0,1,1,1,0);
	else
	{
		m_iCtf = 0;
