...
LINK_ENTITY_TO_CLASS( monster_megret, CMegret );
...
// Touch
//=========================================================
void CMegret :: Touch ( CBaseEntity *pOther )
{
	return;
	Vector		vecSpot;
	TraceResult	tr;
...
void CMegret :: Spawn()
{
	...
	SET_MODEL(ENT(pev), "models/megret.mdl");
UTIL_SetSize(pev, VEC_HULL_MIN, VEC_HULL_MAX);
	pev->classname = MAKE_STRING("monster_megret");
...
//	pev->renderfx		= kRenderFxGlowShell;
	pev->renderamt		= 2;
	pev->rendercolor	= Vector( 1, 1, 255 );
	pev->health			= MEGRET_HEALTH;
...
//=========================================================
void CMegret :: Precache()
{
	PRECACHE_MODEL("models/megret.mdl");

	PRECACHE_SOUND("roach/rch_die.wav");
	PRECACHE_SOUND("roach/rch_walk.wav");
	PRECACHE_SOUND("roach/rch_smash.wav");

	PRECACHE_SOUND("megret/spawn.wav");
	PRECACHE_SOUND("megret/killed.wav");
	PRECACHE_SOUND("megret/aie.wav");
}	

void CMegret :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "megret/aie.wav", 0.8, ATTN_NORM, 0, 120 + RANDOM_LONG(0,20) );
...
// Killed.
//=========================================================
void CMegret :: Killed( entvars_t *pevAttacker, int iGib )
{
	pev->solid = SOLID_NOT;

	//random sound
	
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "megret/killed.wav", 0.8, ATTN_NORM, 0, 80 + RANDOM_LONG(0,39) );
	...
	UTIL_Remove( this );
	g_pGameRules->m_iMegret = 0;
	g_pGameRules->m_flMegretTime = gpGlobals->time + CVAR_GET_FLOAT("mp_megret_time");
	CBasePlayer *peKiller = NULL;
	CBaseEntity *ktmp = CBaseEntity::Instance( pevAttacker );
	if ( ktmp && (ktmp->Classify() == CLASS_PLAYER) )
		peKiller = (CBasePlayer*)ktmp;
	peKiller->AddPoints(REWARD_MEGRET, FALSE);
	if ((CVAR_GET_FLOAT("mp_derrick"))  && (RANDOM_LONG(1, DERRICK_PROB) == 1) && (      ( ktmp && (ktmp->Classify() == CLASS_PLAYER) )         ))
	{
		ClientPrint(peKiller->pev, HUD_PRINTTALK, UTIL_VarArgs("[Derrick]: Tes couilles %s ! Viens faire un tour au bar je t'offrirai un p'tit jaune bien pateux.\n", STRING(peKiller->pev->netname)));
		UTIL_LogPrintf( "Megret retourne en Enfer grace a %s\n",STRING(peKiller->pev->netname)) ;
	}
	else
	{
		UTIL_LogPrintf( "Megret retourne en Enfer\n" ) ;
	}
		MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, pev->origin );
			WRITE_BYTE( TE_EXPLOSION);		
			WRITE_COORD( pev->origin.x );
...
//=========================================================
void CMegret :: MonsterThink( void  )
{
...
//=========================================================
void CMegret :: PickNewDest ( int iCondition )
{
	Vector	vecNewDir;
	Vector	vecDest;
	float	flDist;
...
//=========================================================
void CMegret :: Move ( float flInterval ) 
{
...
// 360 degrees.
//=========================================================
void CMegret :: Look ( int iDistance )
{
...
//=========================================================
//=========================================================
void CHalfLifeMultiplay :: CreateMegret( void )
{
	if (gpGlobals->time < m_flMegretTime) return;
	if ((m_iMegret) || (RANDOM_LONG(0,1)))
	{
	...
	while ( (pSpot = UTIL_FindEntityByClassname(pSpot, "info_megret")) && !m_iMegret )
	{
		if (pSpot)
		if ( pSpot && !RANDOM_LONG(0, 5))
		{
			m_iMegret = 1;
			CBaseEntity *pMegret = CBaseEntity::Create( "monster_megret", pSpot->pev->origin , Vector(0,0,0), NULL );
			UTIL_LogPrintf( "Megret est de retour sous le nom de %s\n", MEGRET_NOM) ;
			UTIL_MessageEffectAll( UTIL_VarArgs( "%s est de retour !", MEGRET_NOM), 1, -1, -1, 0, Vector(0, 255, 0), 255, Vector(0,0,0),0,1,1,1,0);
			if ((CVAR_GET_FLOAT("mp_derrick"))  && (RANDOM_LONG(1, DERRICK_PROB) == 1))
				UTIL_ClientPrintAll(HUD_PRINTTALK, UTIL_VarArgs("[Derrick]: Sus au %s! Que la Derrick's Force soit avec vous.\n", MEGRET_SURNOM));
		}
	}
	if (!m_iMegret)
		m_flMegretTime = gpGlobals->time + CVAR_GET_FLOAT("mp_megret_time");
