...
//pAttacker->AddPoints(REWARD_BLAMEALL, 1 );
		for (int i = 1;i<= gpGlobals->maxClients; i++)
		{
			...
			if (pPlayer4)
			{
			  // UTIL_MessageEffect demande des references, pas
			  // directement des objets -> gcc rale
			  Vector v1(255,255,50);
			  Vector v2(0,0,0);
				UTIL_MessageEffect(pPlayer4,"Les inspecteurs remportent le round",1,-1,-1,0,v1,255,v2,0,1,1,1,0);
...
// MYSQl
/* Fabou : sql commente pour le moment, trop la merde sinon
	char infos[60];
	char infos2[60];
