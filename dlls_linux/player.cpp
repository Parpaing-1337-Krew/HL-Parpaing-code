  ...
  if (nbblame == nbmacon) // tous les macon ont été blamé 
      ...
      for (int i = 1;i<= gpGlobals->maxClients; i++)
	{
	  ...
	  if (pPlayer)
	    {
	      // Fabou
	      Vector v1(255,255,50);
	      Vector v2(0,0,0);
	      UTIL_MessageEffect(pPlayer,"Les inspecteurs remportent le round",1,-1,-1,0,v1,255,v2,0,1,1,1,0);
  ...
  if (m_hActivator->IsPlayer() && this->m_iTeam == INSPECTEUR)
    {
      ...
      ClientPrint(pevAttacker, HUD_PRINTTALK, "NE TAPPEZ PAS SUR LES INSPECTEURS (-20 pts)");
...
void CBasePlayer::SetAnimation( PLAYER_ANIM playerAnim )
{
  ...
  */
  ...
  if ( FBitSet( pev->flags, FL_DUCKING ) )
	...
	{
	  pev->gaitsequence	= 6/*LookupActivity( ACT_CROUCH )*/;
	}
    }
  else if ( speed > 220 )
    {
	  ...
	  if (m_iHasParpaing == 1 && PoseParpaing== false)
	    {
	      ...
	      Vector v1(0,0,200);
	      Vector v2(0,0,0);
	      UTIL_MessageEffect(this,
				 "Vous etes trop blesse pour transporter un parpaing...\n",
				 1,-1,-1,0,v1,255,v2,0,1,1,1,0);
	    }
...
void CBasePlayer::StartDeathCam( void )
{
  ...
  if ( !FNullEnt( pSpot ) )
      ...
      while ( iRand > 0 )
	{
      ...
      //StartObserver( pSpot->v.origin, pev->origin + Vector (0,0,10), Vector(0,0,0),pSpot->v.v_angle );
    }
  else
    {
